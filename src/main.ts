...
setTimeout( () =>{
    // 注销直接生成不了sheet了 直接空白 1.Uncaught TypeError: Cannot read properties of undefined (reading 'getUnitId') 2.chunk-VFGCBB35.js?v=dcb658e3:422 Uncaught Error: Menu item with the same id doc.command.set-inline-format-bold has already been added!
    // univer.dispose()
    // 测试不注销可以重新再生成 但是会报 3.Uncaught TypeError: Cannot read properties of null (reading 'getUnitId')
    univer = new Univer({
        theme: defaultTheme,
        locale: LocaleType.EN_US,
        locales: {
